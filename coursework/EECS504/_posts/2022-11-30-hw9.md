...
We will then use the feature representation that we learned to train an object recognition model (a simple linear classifier) on the 5,000 labeled training images. If the learned representations are useful, we should obtain a performance improvement over only using the small, labeled
training set.
...
```
  ...
  <div id="df-31be1318-6f38-49aa-9a81-5e267a88a2e0">
...
<table border="1" class="dataframe">
  ...
  <tbody>
    ...
    <tr>
      ...
      <button class="colab-df-convert" onclick="convertToInteractive('df-31be1318-6f38-49aa-9a81-5e267a88a2e0')"
  ...
  <style>
    ...
    [theme=dark] .colab-df-convert:hover {
      ...
      <script>
        const buttonEl =
          document.querySelector('#df-31be1318-6f38-49aa-9a81-5e267a88a2e0 button.colab-df-convert');
        buttonEl.style.display =
          google.colab.kernel.accessAllowed ? 'block' : 'none';

        async function convertToInteractive(key) {
          const element = document.querySelector('#df-31be1318-6f38-49aa-9a81-5e267a88a2e0');
          const dataTable =
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_46_1.png)
...
class CLIPModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print("image projection shape: ", image_projected.shape)
        # print("text feature shape: ", text_features.shape)
        # print("text projection shape: ", text_projected.shape)

        criteria = nn.CrossEntropyLoss()

        image_text_matmul = torch.matmul(image_projected, text_projected.t()) / self.temperature
        # print("image projection * text projection shape: ", image_text_matmul.shape)
        loss_it = criteria(image_text_matmul, torch.arange(image_text_matmul.shape[0], device = device))

        text_image_matmul = torch.matmul(text_projected, image_projected.t()) / self.temperature
        # print("text projection * image projection shape: ", text_image_matmul.shape)
        loss_ti = criteria(text_image_matmul, torch.arange(text_image_matmul.shape[0], device = device))

        #loss = torch.tensor([loss_it, loss_ti], requires_grad=True)
        loss = (loss_it + loss_ti) / 2
        ...
        # loss_it = loss(image_fea)
        # numerator = torch.exp((image_projected * text_projected) / self.temperature)

        
        # denum_it = torch.sum(torch.exp(torch.matmul(image_projected.t(), text_projected) / self.temperature), axis = 1)
        # loss_it = -torch.sum(torch.log(numerator / denum_it)) / numerator.shape[0]

        # denum_ti = torch.sum(torch.exp(torch.matmul(text_projected.t(), image_projected) / self.temperature), axis = 1)
        # loss_ti = -torch.sum(torch.log(numerator / denum_ti)) / numerator.shape[0]

        # loss = torch.tensor([loss_it, loss_ti])

        # print("numerator: ", numerator.shape)
        # print("denum it: ", denum_it.shape)
        # print("denum ti: ", denum_ti.shape)
        # print("loss: ", loss)
...
```
    ...
    loss: 0.11498111486434937
    loss: 0.07898600399494171
    loss: 0.07101263105869293
    ...
    loss: 0.04536661505699158
    loss: 0.050881966948509216
    loss: 0.03583340346813202
    ...
    loss: 0.0393420085310936
    loss: 0.028501039370894432
    loss: 0.025959571823477745
...
```
    ...
    Epoch 0, loss 53.551, train accuracy 0.6498
    Epoch 10, loss 8.225, train accuracy 0.9636
    Epoch 20, loss 4.023, train accuracy 0.9900000000000001
    Epoch 30, loss 2.499, train accuracy 0.9978
    Epoch 40, loss 1.660, train accuracy 0.9992000000000001
    Epoch 50, loss 1.218, train accuracy 0.9998
    Epoch 60, loss 0.935, train accuracy 1.0
    Epoch 70, loss 0.754, train accuracy 1.0
    Epoch 80, loss 0.630, train accuracy 1.0
    Epoch 90, loss 0.542, train accuracy 1.0
...
**Autoencoder-based**
method 1 accuracy: 0.31   
method 2 accuracy: 0.41  
method 3 accuracy: 0.25   
...
def retrieve_images(model, image_embeddings, query, image_filenames, k=9):
    ...
    text_image_matmul = torch.matmul(text_embeddings, image_embeddings.t())
    indexes = torch.topk(text_image_matmul, k)[1].tolist()
    matches = image_filenames[indexes]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_72_1.png)
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_73_1.png)
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_74_1.png)
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_75_1.png)
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_76_1.png)
...
![png](/assets/images/coursework/EECS504/hw9/hw9_upload_77_1.png)
