...
- Generate a random sample $X_{1}^{*}, ..., X_{n}^{*}$ from $\hat F_n$. This will be called a bootstrap sample from $X_1, ..., X_n$. This is equivalent to drawing with replacement from $X_1, ..., X_n$ 
- Compute $\hat \theta^* = S(X_{1}^{*}, ..., X_{n}^{*})$
